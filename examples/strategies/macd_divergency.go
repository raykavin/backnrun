...
func (m *MACDDivergenceStrategy) OnCandle(df *core.Dataframe, broker core.Broker) {
	pair := df.Pair
	...
	if assetPosition > 0 {
		...
		if bearishDivergence && m.isMACDCrossingDown(df) {
			m.executeExit(df, broker, assetPosition)
		}
	} else {
		// Sem posição, verificar entrada
		if bullishDivergence && m.isMACDCrossingUp(df) {
			m.executeBuy(df, broker, quotePosition, closePrice)
		}
	}
}
...
func (m *MACDDivergenceStrategy) executeExit(df *core.Dataframe, broker core.Broker, assetPosition float64) {
	pair := df.Pair
...
func (m *MACDDivergenceStrategy) executeBuy(df *core.Dataframe, broker core.Broker, quotePosition, closePrice float64) {
	pair := df.Pair
