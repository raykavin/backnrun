...
func (s *CrossEMAMACD) OnCandle(df *core.Dataframe, broker core.Broker) {
	...
	// hasShortPosition is only used if short trading is enabled
	...
	if hasPosition {
		...
		if hasLongPosition && s.shouldExitLong(df) {
			s.executeExitLong(df, broker, assetPosition)
			return
		}

		// If we have a short position, check if we should exit
		// Note: Uncomment this if your broker supports short positions
		/*
			if hasShortPosition && s.shouldExitShort(df) {
				s.executeExitShort(df, broker, assetPosition)
				return
			}
		*/
	...
	if s.shouldEnterLong(df, quotePosition) {
		s.executeEnterLong(df, broker, closePrice, quotePosition)
		return
	}

	// Check for sell signal (short entry)
	// Note: Uncomment this if your broker supports short positions
	/*
		if s.shouldEnterShort(df) {
			s.executeEnterShort(df, broker, closePrice)
			return
		}
	*/
}
...
func (s *CrossEMAMACD) executeEnterLong(df *core.Dataframe, broker core.Broker, closePrice, quotePosition float64) {
...
func (s *CrossEMAMACD) executeExitLong(df *core.Dataframe, broker core.Broker, assetPosition float64) {
...
func (s *CrossEMAMACD) executeEnterShort(df *core.Dataframe, broker core.Broker, closePrice float64) {
	...
	_, err := broker.CreateOrderMarket(core.SideTypeSell, df.Pair, amount)
	if err != nil {
		backnrun.DefaultLog.WithFields(map[string]interface{}{
			"pair":  df.Pair,
			"side":  core.SideTypeSell,
...
func (s *CrossEMAMACD) executeExitShort(df *core.Dataframe, broker core.Broker, assetPosition float64) {
	// Note: For short positions, assetPosition will be negative, so we need to use the absolute value
	_, err := broker.CreateOrderMarket(core.SideTypeBuy, df.Pair, -assetPosition)
	if err != nil {
		backnrun.DefaultLog.WithFields(map[string]interface{}{
			"pair": df.Pair,
			"side": core.SideTypeBuy,
