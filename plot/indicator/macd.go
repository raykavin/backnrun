...
func (m *macd) Load(df *core.Dataframe) {
	warmup := m.Warmup()
	if !ValidateDataframe(df, warmup) {
		return
	}

	macdLine, signalLine, histogram := talib.Macd(df.Close, m.Fast, m.Slow, m.Signal)
	m.ValuesMACD, m.Time = TrimData(macdLine, df.Time, warmup)
	m.ValuesMACDSignal, _ = TrimData(signalLine, df.Time, warmup)
	m.ValuesMACDHist, _ = TrimData(histogram, df.Time, warmup)
}
