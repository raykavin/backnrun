...
func TestPaperWallet_OrderLimit(t *testing.T) {
	t.Run("normal order", func(t *testing.T) {
		...
		order, err = wallet.CreateOrderLimit(core.SideTypeSell, "BTCUSDT", 1, 200)
		require.NoError(t, err)
	...
	t.Run("multiple pending orders", func(t *testing.T) {
		...
		order, err = wallet.CreateOrderLimit(core.SideTypeBuy, "BTCUSDT", 1, 20)
		require.NoError(t, err)
		require.NotEmpty(t, order)
		...
		order, err = wallet.CreateOrderLimit(core.SideTypeBuy, "BTCUSDT", 1, 50)
		require.NoError(t, err)
		require.NotEmpty(t, order)
		...
		order, err = wallet.CreateOrderLimit(core.SideTypeSell, "BTCUSDT", 2, 40)
		require.NoError(t, err)
		require.NotEmpty(t, order)
...
func TestPaperWallet_OrderMarket(t *testing.T) {
	...
	// insufficient funds
	order, err = wallet.CreateOrderMarket(core.SideTypeBuy, "BTCUSDT", 100)
	require.Equal(t, &OrderError{
		Err:      ErrInsufficientFunds,
		Pair:     "BTCUSDT",
		Quantity: 100}, err)
	...
	// sell
	wallet.OnCandle(core.Candle{Pair: "BTCUSDT", Close: 100})
	order, err = wallet.CreateOrderMarket(core.SideTypeSell, "BTCUSDT", 1)
	require.NoError(t, err)
	require.Equal(t, 1.0, order.Quantity)
	require.Equal(t, 100.0, order.Price)
...
func TestPaperWallet_OrderOCO(t *testing.T) {
	...
	orders, err := wallet.CreateOrderOCO(core.SideTypeSell, "BTCUSDT", 1, 100, 40, 39)
	require.NoError(t, err)
