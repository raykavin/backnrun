...
func NewFutures(ctx context.Context, logger logger.Logger, options ...FuturesOption) (*Futures, error) {
	...
	// Test connection
	err := futures.client.NewPingService().Do(ctx)
	if err != nil {
		return nil, fmt.Errorf("binance futures ping fail: %w", err)
	}

	// Get exchange info
	exchangeInfo, err := futures.client.NewExchangeInfoService().Do(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get futures exchange info: %w", err)
	}

	// Set leverage and margin type for configured pairs
	for _, option := range futures.pairOptions {
		_, err = futures.client.NewChangeLeverageService().
			Symbol(option.Pair).
			Leverage(option.Leverage).
			Do(ctx)
		if err != nil {
			return nil, fmt.Errorf("failed to set leverage for %s: %w", option.Pair, err)
		}

		err = futures.client.NewChangeMarginTypeService().
			Symbol(option.Pair).
		...
		if err != nil {
			...
			if apiError, ok := err.(*common.APIError); !ok || apiError.Code != ErrNoNeedChangeMarginType {
				return nil, fmt.Errorf("failed to set margin type for %s: %w", option.Pair, err)
			}
		}
	}
...
func (f *Futures) AssetsInfo(pair string) core.AssetInfo {
	return f.assetsInfo[pair]
}
...
func (f *Futures) CreateOrderOCO(_ core.SideType, _ string, _, _, _, _ float64) ([]core.Order, error) {
	return nil, fmt.Errorf("OCO orders not supported in futures market")
}

// CreateOrderStop creates a stop-loss order
func (f *Futures) CreateOrderStop(pair string, quantity, limit float64) (core.Order, error) {
...
func (f *Futures) CreateOrderLimit(side core.SideType, pair string,
...
func (f *Futures) CreateOrderMarket(side core.SideType, pair string, quantity float64) (core.Order, error) {
	err := f.validate(pair, quantity)
	if err != nil {
		return core.Order{}, err
	}
...
// This is not implemented in futures
func (f *Futures) CreateOrderMarketQuote(_ core.SideType, _ string, _ float64) (core.Order, error) {
	return core.Order{}, fmt.Errorf("market quote orders not supported in futures market")
}

// Cancel cancels an order
func (f *Futures) Cancel(order core.Order) error {
...
func (f *Futures) Orders(pair string, limit int) ([]core.Order, error) {
	...
	for _, order := range result {
		orders = append(orders, convertFuturesOrder(order))
	}
	return orders, nil
}

// Order gets a specific order by ID
func (f *Futures) Order(pair string, id int64) (core.Order, error) {
	...
	return convertFuturesOrder(order), nil
}

// convertFuturesOrder converts a Binance futures order to a core.Order
func convertFuturesOrder(order *futures.Order) core.Order {
	var price float64
...
func (f *Futures) Account() (core.Account, error) {
	acc, err := f.client.NewGetAccountService().Do(f.ctx)
	if err != nil {
...
func (f *Futures) Position(pair string) (asset, quote float64, err error) {
