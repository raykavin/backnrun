...
func (w *Williams91Strategy) OnCandle(ctx context.Context, df *core.Dataframe, broker core.Broker) {
	pair := df.Pair
	...
	if assetPosition > 0 {
		if w.shouldExit(pair, df) {
			w.executeExit(ctx, df, broker, assetPosition)
			return
		}
	}
	...
	if w.shouldEnter(closePrice, lowPrice, lowestLow, assetPosition) {
		w.executeEntry(ctx, df, broker, quotePosition, atr)
	}
}
...
func (w *Williams91Strategy) executeEntry(ctx context.Context, df *core.Dataframe, broker core.Broker, quotePosition, atr float64) {
	pair := df.Pair
	closePrice := df.Close.Last(0)
...
func (w *Williams91Strategy) executeExit(ctx context.Context, df *core.Dataframe, broker core.Broker, assetPosition float64) {
	pair := df.Pair
