...
func (s *AdaptiveMomentumStrategy) OnCandle(ctx context.Context, df *core.Dataframe, broker core.Broker) {
	pair := df.Pair
	...
	if assetPosition > 0 && s.shouldExit(df) {
		s.executeExit(ctx, df, broker, assetPosition, "Regular Exit")
		return
	}
...
func (s *AdaptiveMomentumStrategy) executeEntry(ctx context.Context, df *core.Dataframe, broker core.Broker, quotePosition, closePrice float64) {
	pair := df.Pair
...
func (s *AdaptiveMomentumStrategy) executeExit(ctx context.Context, df *core.Dataframe, broker core.Broker, assetPosition float64, reason string) {
	pair := df.Pair
