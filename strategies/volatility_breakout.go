...
func (s *VolatilityBreakoutStrategy) OnCandle(ctx context.Context, df *core.Dataframe, broker core.Broker) {
	pair := df.Pair
	...
	if assetPosition > 0 {
		...
		if s.checkTrailingStop(pair, df) {
			s.executeExit(ctx, df, broker, assetPosition, "Trailing Stop")
			return
		}

		// Check for take profit
		if s.checkTakeProfit(pair, closePrice) {
			s.executeExit(ctx, df, broker, assetPosition, "Take Profit")
			return
		}

		// Check for stop loss
		if s.checkStopLoss(pair, lowPrice) {
			s.executeExit(ctx, df, broker, assetPosition, "Stop Loss")
			return
		}
	}
	...
	if assetPosition == 0 && len(s.activePositions) < s.maxPositions {
		...
		if s.isVolatilityContraction(pair) && s.isBreakout(df, highPrice) {
			...
			if s.checkTrendFilters(df) {
				s.executeEntry(ctx, df, broker, quotePosition, closePrice)
			}
		}
	}
}
...
func (s *VolatilityBreakoutStrategy) executeEntry(ctx context.Context, df *core.Dataframe, broker core.Broker, quotePosition, closePrice float64) {
	pair := df.Pair
	...
	// Execute market buy
	_, err := broker.CreateOrderMarket(ctx, core.SideTypeBuy, pair, positionSize)
	if err != nil {
	...
	// Set stop loss order
	stopOrder, err := broker.CreateOrderStop(ctx, pair, positionSize, stopLossPrice)
	if err != nil {
		bot.DefaultLog.WithFields(map[string]any{
			"pair":      pair,
			"size":      positionSize,
...
func (s *VolatilityBreakoutStrategy) executeExit(ctx context.Context, df *core.Dataframe, broker core.Broker, assetPosition float64, reason string) {
	pair := df.Pair
